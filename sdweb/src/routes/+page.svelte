<script>
	...
	const toggleModal = (e) => {
		...
		modal = !modal;
		if (e.target.id === 'hwb' || e.target.id === 'dx3') {
			content = e.target.id;
		} else {
			content = 'survey';
		}
...
{#if modal}
	<div
		...
		class="fixed top-0 left-0 h-screen w-full bg-black/50 p-8 lg:p-32 flex place-content-center"
	>
		<div class="bg-zinc-100 flex flex-col gap-2 p-6 h-fit text-sm lg:text-base">
			{#if content === 'hwb'}
